...
export async function sendOrderConfirmation(
  ...
  const itemRows = order.items.map((item) =>
    `<tr>
      <td style="padding:8px 0; border-bottom:1px solid #fce7f3;">${item.nameVi}</td>
      <td style="padding:8px 0; border-bottom:1px solid #fce7f3; text-align:center;">x${item.qty}</td>
      <td style="padding:8px 0; border-bottom:1px solid #fce7f3; text-align:right;">
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:'Lato',sans-serif; max-width:560px; margin:0 auto; color:#3d0b22;">
        <div style="background:linear-gradient(135deg,#ec4899,#be185d); padding:32px; text-align:center;">
        ...
        <div style="padding:32px; background:#fffbf7;">
          ...
          <table style="width:100%; border-collapse:collapse; margin-bottom:20px;">
            <thead>
              <tr style="border-bottom:2px solid #ec4899;">
          ...
          <div style="text-align:right; border-top:2px solid #ec4899; padding-top:12px;">
            <p style="margin:4px 0; font-size:13px;">Phí vận chuyển: <strong>${fmt(order.shippingFee)}</strong></p>
            <p style="margin:4px 0; font-size:16px; color:#be185d;">
              Tổng thanh toán: <strong>${fmt(order.total)}</strong>
            </p>
          </div>
          <div style="margin-top:24px; padding:16px; background:#fdf5f0; border-radius:8px; font-size:13px;">
        ...
        <div style="padding:16px; background:#fce7f3; text-align:center; font-size:11px; color:#be185d;">
...
export async function sendDispatchNotification(
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:'Lato',sans-serif; max-width:560px; margin:0 auto; color:#3d0b22; padding:32px; background:#fffbf7;">
        <h2 style="font-family:serif; font-weight:300; color:#be185d;">Đơn hàng đang được vận chuyển ✿</h2>
        <p>Đơn hàng <strong>${orderNumber}</strong> của bạn đã được bàn giao cho đơn vị vận chuyển.</p>
        <p>Mã vận đơn: <strong style="color:#ec4899; font-size:18px;">${trackingCode}</strong></p>
...
export async function sendConsultationNotification(data: {
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:'Lato',sans-serif; max-width:560px; margin:0 auto; color:#3d0b22;">
        <div style="background:linear-gradient(135deg,#ec4899,#be185d); padding:28px 32px; text-align:center;">
          <h1 style="color:white; font-family:serif; font-weight:300; margin:0; font-size:24px;">
            ✿ Yêu cầu tư vấn hoa mới
          </h1>
        </div>
        <div style="padding:32px; background:#fffbf7; border:1px solid #fce7f3;">
          <table style="width:100%; border-collapse:collapse;">
            <tr>
              <td style="padding:10px 0; border-bottom:1px solid #fce7f3; width:140px;">
            ...
            <tr>
              ...
              <td style="padding:10px 0; border-bottom:1px solid #fce7f3;">
                <a href="mailto:${data.email}" style="color:#be185d;">${data.email}</a>
              </td>
            </tr>
            ${data.deliveryDate ? `
            <tr>
          ...
          <div style="margin-top:24px; text-align:center;">
            <a href="${process.env.ADMIN_URL ?? "http://localhost:3001"}/dashboard/consultations"
              style="display:inline-block; padding:12px 28px; background:linear-gradient(135deg,#f472b6,#be185d); color:white; text-decoration:none; border-radius:24px; font-size:13px; font-weight:600; letter-spacing:0.05em;">
              Xem trong Admin →
            </a>
          </div>
        </div>
        <div style="padding:14px; background:#fce7f3; text-align:center; font-size:11px; color:#be185d;">
          Hanami — Đèn hoa cưới thủ công · hanami.vn
        </div>
      </div>
    `,
